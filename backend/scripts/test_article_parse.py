#!/usr/bin/env python3
"""
Test Article Parse Service with Real LLM
"""
import sys
import os
...
def test_article_parse():
    print("=" * 70)
    print("Test Article Parse Service (Real LLM)")
    print("=" * 70)
    print()
...
RAG 技术在企业级应用中展现出强大的实用价值，特别适合需要准确性和可追溯性的场景，如客户服务、技术文档查询、法律咨询等领域。这使得 RAG 成为企业级 AI 应用的重要技术选择。"""
    ...
    print("Parsing article with LLM...")
    print()

    try:
        service = ArticleParseService()
        analysis = service.parse_article_with_retry(article)
        ...
        print(f"Estimated Duration: {analysis.estimated_video_duration}s")
